...
##Process data
candidatesDATA <-
    cnAnalysis450k::createBinsFast(samples, ctrl,  ctrlAll, binsize = 200000)
candidatesMATRIX <- 
    cnAnalysis450k::createBinMatrix(candidatesDATA[which(candidatesDATA$p.val <= 1), ], "median")
candidatesCUT <-
...
##Display
candidatesFINAL <- read.csv("~/phd/test.csv", row.names=1)
anno_row <- data.frame(do.call(rbind, strsplit(rownames(candidatesFINAL), ":")))
colnames(anno_row) <- c("startCG", "chr", "pos")
rownames(anno_row) <- rownames(candidatesFINAL)
pheatmap::pheatmap(candidatesFINAL, annotation_row=anno_row[,"chr",drop=F],
